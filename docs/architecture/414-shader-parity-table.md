...
| Legacy material class3 | 3 | `class1/deferred/materialV.glsl` + `class3/deferred/materialF.glsl` | `vulkan/final/class1/deferred/material.vert` + `vulkan/final/class3/deferred/material.frag` / `material_gbuffer*.frag` | Runtime legacy Material G-buffer now binds `class3/deferred/material_gbuffer*.frag` with the current Vulkan world push-constant ABI; direct Material still uses the active runtime fragment. | Replace the direct Material adapter only after the forward/alpha material pass has a matching final shader interface. |
| PBR opaque deferred | 1 | `class1/deferred/pbropaqueV.glsl` + `pbropaqueF.glsl` | `vulkan/final/class1/deferred/pbropaque.vert` + `pbropaque.frag` / `pbropaque_gbuffer*.frag` | Runtime PBR G-buffer now binds `class1/deferred/pbropaque_gbuffer*.frag` with the current Vulkan world push-constant ABI; direct PBR still uses the active runtime fragment. | Replace the direct PBR adapter after the forward/HUD PBR path has a matching final shader interface, then compare base color, normal, ORM, emissive, and reflection-probe terms. |
