...
| Haze | 2/3 | `class2/deferred/softenLightV.glsl` + `class3/deferred/hazeF.glsl` | `vulkan/final/class2/deferred/soften_light.vert` + `vulkan/final/class3/deferred/haze.frag`; runtime owner `haze_runtime.frag` | Runtime haze now binds `class3/deferred/haze_runtime.frag`, which is the current active approximation isolated from the final class3 inventory shader. It still differs strongly from OpenGL. | OpenGL source-level reference exists; strict PPM currently fails with mean diff `124.5276`, max diff `142`. Replace this runtime owner only after the haze/depth/water-exclusion interface is faithful. |
