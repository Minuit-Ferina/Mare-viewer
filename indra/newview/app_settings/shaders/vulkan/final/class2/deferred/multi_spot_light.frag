...
// Faithful source-level Vulkan port of class2/deferred/multiSpotLightF.glsl.
// Class 2 adds projected-light shadow modulation to the legacy multi-spot path.
...
layout(set = 1, binding = 0) uniform MareMultiSpotLightClass2Uniforms
{
    mat4 inv_proj;
    mat4 proj_mat;
...
void main()
{
        ...
        proj_tc.y > 0.0)
    {
        ...
        if (da > 0.0)
        {
            ...
            amb_da += (da * 0.5 + 0.5) * u.proj_lod_range_ambiance.w;
        }
        ...
        col += amb_da * u.color_screen_x.rgb * diff_tex.rgb * amb_plcol.rgb * amb_plcol.a;
    }
