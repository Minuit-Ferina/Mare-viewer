...
vec3 pbr_lighting(vec3 base_color, vec2 texcoord)
{
    ...
    vec3 ambient = base_color * get_scene_ambient_color(0.34) * occlusion;
    vec3 direct_color = get_scene_direct_color();
    vec3 direct = base_color * direct_color * ndotl * mix(1.0, 0.5, metallic);
    float spec_power = mix(96.0, 8.0, roughness);
    vec3 f0 = mix(vec3(0.04), base_color, metallic);
    vec3 specular = f0 * direct_color * pow(ndoth, spec_power) * (1.0 - roughness);
    vec3 environment = base_color * clamp(pc.material_legacy.a, 0.0, 1.0) * 0.16;

    return ambient + direct + specular + environment;
}
...
void main()
{
    ...
    color.rgb = pbr_lighting(color.rgb, vary_material_texcoord0.xy) + emissive;
