...
void main()
{
    vec4 diffuse = diffuse_lookup(vary_texcoord0.xy);
    float glow_alpha = diffuse.a * vertex_color.a;
    vec3 glow_color = diffuse.rgb * vertex_color.rgb;

    if (has_material_flag(MATERIAL_GLTF_PBR))
    {
        float gltf_mode = pc.material_modes.y;
        if (gltf_mode > 1.5 && pc.params.x >= 0.0 && diffuse.a < pc.params.x)
        {
            discard;
        }
        ...
        glow_color = emissive;
        glow_alpha = max(max(emissive.r, emissive.g), emissive.b) * vertex_color.a;
    }

    frag_color = vec4(max(glow_color, vec3(0.0)), max(glow_alpha, 0.0));
}
