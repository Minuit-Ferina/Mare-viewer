...
vec4 apply_gbuffer_alpha_policy(vec4 color)
{
...
void main()
{
    ...
    if (!has_material_flag(MATERIAL_WATER))
    {
        color = apply_gbuffer_alpha_policy(color);
    }
    else if (pc.params.x >= 0.0 && color.a < pc.params.x)
    {
        discard;
    }
    ...
    float env_intensity =
        has_material_flag(MATERIAL_FULLBRIGHT) ? 1.0 : pc.material_legacy.a;
    frag_diffuse = vec4(max(color.rgb, vec3(0.0)), env_intensity);
