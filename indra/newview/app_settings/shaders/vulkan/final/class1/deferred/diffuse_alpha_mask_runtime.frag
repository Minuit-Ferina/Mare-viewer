...
vec4 apply_mask_alpha_policy(vec4 color)
{
    ...
    const bool emissive_mode =
        !has_material_flag(MATERIAL_GLTF_PBR) &&
        alpha_mode > 2.5 &&
        alpha_mode < 3.5;

    if (mask_mode || pc.params.x >= 0.0)
    {
...
vec3 mask_lighting(vec3 base_color, vec2 texcoord)
{
...
void main()
{
    ...
    color = apply_mask_alpha_policy(color);
    ...
    frag_color = vec4(mask_lighting(color.rgb, vary_material_texcoord0.xy) + emissive, color.a);
}
